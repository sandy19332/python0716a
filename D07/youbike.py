...
def getYoubikes() ->list:
    limit = 500
    path ='https://data.tycg.gov.tw/api/v1/rest/datastore/a1b4714b-3b75-4ff8-a8f2-cc377e4eaa0f?format=json&json=%d'
    path = path % limit
    data = requests.get(path).text #取得網路原始字串資料
    dict = json.loads(data) #將 json字串模式轉成 Python 可用的物件
